...
function exif_content_get_entry(content, tag)
    ccall((:exif_content_get_entry, libexifcontent), Ptr{ExifEntry}, (Ptr{ExifContent}, ExifTag), content, tag)
end
...
function exif_content_new_mem(arg1)
    ccall((:exif_content_new_mem, libexifcontent), Ptr{ExifContent}, (Ptr{ExifMem},), arg1)
end
...
function exif_content_add_entry(c, entry)
    ccall((:exif_content_add_entry, libexifcontent), Cvoid, (Ptr{ExifContent}, Ptr{ExifEntry}), c, entry)
end

function exif_content_remove_entry(c, e)
    ccall((:exif_content_remove_entry, libexifcontent), Cvoid, (Ptr{ExifContent}, Ptr{ExifEntry}), c, e)
end
...
function exif_content_get_ifd(c)
    ccall((:exif_content_get_ifd, libexifcontent), ExifIfd, (Ptr{ExifContent},), c)
end
...
function exif_content_log(content, log)
    ccall((:exif_content_log, libexifcontent), Cvoid, (Ptr{ExifContent}, Ptr{ExifLog}), content, log)
end
...
function exif_data_new_mem(arg1)
    ccall((:exif_data_new_mem, libexifdata), Ptr{ExifData}, (Ptr{ExifMem},), arg1)
end
...
function exif_data_get_byte_order(data)
    ccall((:exif_data_get_byte_order, libexifdata), ExifByteOrder, (Ptr{ExifData},), data)
end

function exif_data_set_byte_order(data, order)
    ccall((:exif_data_set_byte_order, libexifdata), Cvoid, (Ptr{ExifData}, ExifByteOrder), data, order)
end
...
function exif_data_get_mnote_data(d)
    ccall((:exif_data_get_mnote_data, libexifdata), Ptr{ExifMnoteData}, (Ptr{ExifData},), d)
end
...
function exif_data_set_data_type(d, dt)
    ccall((:exif_data_set_data_type, libexifdata), Cvoid, (Ptr{ExifData}, ExifDataType), d, dt)
end

function exif_data_get_data_type(d)
    ccall((:exif_data_get_data_type, libexifdata), ExifDataType, (Ptr{ExifData},), d)
end
...
function exif_data_log(data, log)
    ccall((:exif_data_log, libexifdata), Cvoid, (Ptr{ExifData}, Ptr{ExifLog}), data, log)
end
...
function exif_entry_new_mem(arg1)
    ccall((:exif_entry_new_mem, libexifentry), Ptr{ExifEntry}, (Ptr{ExifMem},), arg1)
end
...
function exif_entry_initialize(e, tag)
    ccall((:exif_entry_initialize, libexifentry), Cvoid, (Ptr{ExifEntry}, ExifTag), e, tag)
end
...
function exif_loader_new_mem(mem)
    ccall((:exif_loader_new_mem, libexifloader), Ptr{ExifLoader}, (Ptr{ExifMem},), mem)
end
...
function exif_loader_get_data(loader)
    ccall((:exif_loader_get_data, libexifloader), Ptr{ExifData}, (Ptr{ExifLoader},), loader)
end
...
function exif_loader_log(loader, log)
    ccall((:exif_loader_log, libexifloader), Cvoid, (Ptr{ExifLoader}, Ptr{ExifLog}), loader, log)
end
...
function exif_log_new_mem(arg1)
    ccall((:exif_log_new_mem, libexiflog), Ptr{ExifLog}, (Ptr{ExifMem},), arg1)
end
...
function exif_mem_alloc(m, s)
    ccall((:exif_mem_alloc, libexifmem), Ptr{Cvoid}, (Ptr{ExifMem}, ExifLong), m, s)
end

function exif_mem_realloc(m, p, s)
    ccall((:exif_mem_realloc, libexifmem), Ptr{Cvoid}, (Ptr{ExifMem}, Ptr{Cvoid}, ExifLong), m, p, s)
end
...
function exif_mnote_data_log(arg1, arg2)
    ccall((:exif_mnote_data_log, libexifmnotedata), Cvoid, (Ptr{ExifMnoteData}, Ptr{ExifLog}), arg1, arg2)
end
...
function exif_tag_get_name_in_ifd(tag, ifd)
    ccall((:exif_tag_get_name_in_ifd, libexiftag), Ptr{Cchar}, (ExifTag, ExifIfd), tag, ifd)
end

function exif_tag_get_title_in_ifd(tag, ifd)
    ccall((:exif_tag_get_title_in_ifd, libexiftag), Ptr{Cchar}, (ExifTag, ExifIfd), tag, ifd)
end

function exif_tag_get_description_in_ifd(tag, ifd)
    ccall((:exif_tag_get_description_in_ifd, libexiftag), Ptr{Cchar}, (ExifTag, ExifIfd), tag, ifd)
end

function exif_tag_get_support_level_in_ifd(tag, ifd, t)
    ccall((:exif_tag_get_support_level_in_ifd, libexiftag), ExifSupportLevel, (ExifTag, ExifIfd, ExifDataType), tag, ifd, t)
end
...
function exif_get_short(b, order)
    ccall((:exif_get_short, libexifutils), ExifShort, (Ptr{Cuchar}, ExifByteOrder), b, order)
end

function exif_get_sshort(b, order)
    ccall((:exif_get_sshort, libexifutils), ExifSShort, (Ptr{Cuchar}, ExifByteOrder), b, order)
end

function exif_get_long(b, order)
    ccall((:exif_get_long, libexifutils), ExifLong, (Ptr{Cuchar}, ExifByteOrder), b, order)
end

function exif_get_slong(b, order)
    ccall((:exif_get_slong, libexifutils), ExifSLong, (Ptr{Cuchar}, ExifByteOrder), b, order)
end

function exif_get_rational(b, order)
    ccall((:exif_get_rational, libexifutils), ExifRational, (Ptr{Cuchar}, ExifByteOrder), b, order)
end

function exif_get_srational(b, order)
    ccall((:exif_get_srational, libexifutils), ExifSRational, (Ptr{Cuchar}, ExifByteOrder), b, order)
end

function exif_set_short(b, order, value)
    ccall((:exif_set_short, libexifutils), Cvoid, (Ptr{Cuchar}, ExifByteOrder, ExifShort), b, order, value)
end

function exif_set_sshort(b, order, value)
    ccall((:exif_set_sshort, libexifutils), Cvoid, (Ptr{Cuchar}, ExifByteOrder, ExifSShort), b, order, value)
end

function exif_set_long(b, order, value)
    ccall((:exif_set_long, libexifutils), Cvoid, (Ptr{Cuchar}, ExifByteOrder, ExifLong), b, order, value)
end

function exif_set_slong(b, order, value)
    ccall((:exif_set_slong, libexifutils), Cvoid, (Ptr{Cuchar}, ExifByteOrder, ExifSLong), b, order, value)
end

function exif_set_rational(b, order, value)
    ccall((:exif_set_rational, libexifutils), Cvoid, (Ptr{Cuchar}, ExifByteOrder, ExifRational), b, order, value)
end

function exif_set_srational(b, order, value)
    ccall((:exif_set_srational, libexifutils), Cvoid, (Ptr{Cuchar}, ExifByteOrder, ExifSRational), b, order, value)
end
...
function exif_array_set_byte_order(arg1, arg2, arg3, o_orig, o_new)
    ccall((:exif_array_set_byte_order, libexifutils), Cvoid, (ExifFormat, Ptr{Cuchar}, Cuint, ExifByteOrder, ExifByteOrder), arg1, arg2, arg3, o_orig, o_new)
end
